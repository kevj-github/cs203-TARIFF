  ...
  const handleDelete = async (p: Product) => {
    ...
    try {
      ...
      setAlert({
        ...
        message: `"${p.name}" was deleted successfully.`,
      });
    } catch (e: any) {
  ...
  const handleCreate = async () => {
    ...
    if (!newProduct.name || !newProduct.productType || !newProduct.hsCode) {
      setAlert({
        ...
        message:
          "Please fill in Name, Category, and HS Code before submitting.",
      });
      return;
    }
    ...
    <div className="w-full p-6">
      <AnimatePresence>
        {alert && (
          ...
          >
            <Alert
              className={`border-l-4 flex items-start gap-3 p-3 ${
                alert.type === "error"
            ...
            >
              {/* Icon */}
              {alert.type === "error" ? (
                <FileWarning className="h-5 w-5 mt-0.5 text-red-600 shrink-0" />
              ) : (
